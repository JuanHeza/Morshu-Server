...
var (
	...
	secret, err     = hex.DecodeString(os.Getenv("SECURE_STRING"))
	ErrInvalidValue = errors.New("invalid cookie value")
	ErrValueTooLong = errors.New("cookie value too long")
	sessionKey      = os.Getenv("SESSION_KEY")
)

func SetCookieHandler(w *http.ResponseWriter, r *http.Request, user model.User) {
...
func CleanCookieHandler(w *http.ResponseWriter, r *http.Request) {
	// Call buf.String() to get the gob-encoded value as a string and set it as
	// the cookie value.
	cookie := http.Cookie{
		Name:   sessionKey,
...
func GetCookieHandler(w http.ResponseWriter, r *http.Request) {
	...
	gobEncodedValue, err := ReadEncrypted(r, sessionKey, secret)
	if err != nil {
		switch {
...
func WriteEncrypted(w *http.ResponseWriter, cookie http.Cookie, secret []byte) error {
	...
	cookie.Value = base64.URLEncoding.EncodeToString([]byte(string(encryptedValue)))
